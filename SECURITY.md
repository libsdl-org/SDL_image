...
- Trigger file: `tools/sdl_image_asan/out/apng_missing_fctl_oob.apng`
...
A plausible fix would be to validate APNG structural consistency after parsing and before frame access. In practice, the decoder should reject files where `acTL.num_frames` exceeds the number of successfully parsed `fcTL` entries, and `IMG_AnimationDecoderGetNextFrame_Internal` should refuse to index `ctx->fctl_frames[current_frame_index]` unless that frame metadata was actually initialized.
