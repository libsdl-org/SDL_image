...
### Attached PoC artifacts

- Harness source: `tools/sdl_image_asan/harness.c`
- Harness build script: `tools/sdl_image_asan/build_harness.sh`
- PoC generator: `tools/sdl_image_asan/make_pocs.py`
- Trigger file: `tools/sdl_image_asan/out/apng_chunk_wrap_trigger.png`
...
A plausible fix would be to reject oversized chunk lengths before computing or allocating `chunk_size`, and to perform the size calculation in a type that cannot silently wrap. Concretely, `read_png_chunk` should validate that `data_length` is small enough for `sizeof(header) + data_length + 4`, fail closed on overflow, and avoid any read or write until that validation succeeds.

No patch or linked PR is included with this report yet.
